...
export default function Home({ allTestimonials, homeProjects }: HomeProps) {
	...
	return (
		<>
          ...
          /> */}
					<Portfolio id='portfolio' homeProjects={homeProjects} setPortfolioInView={setPortfolioInView} />
					<CallToAction />
					<Footer />
				</HomeContent>
			</HomeContainer>
...
export const getStaticProps: GetStaticProps = async () => {
	try {
		const { allTestimonials, allProjects: homeProjects }: ResponseDataProps = await DatoCmsRequest({
			query: GET_HOME,
			variables: {
				first: 3,
			},
